...
const CartItems = (props) => {
    const cartItems = useSelector(selectRecommend);
    console.log(cartItems);
    const userEmail = useSelector(selectUserEmail);
    const itemNum = cartItems.length;

    function onRemove(course) {
        db.collection(userEmail + " cart").doc(course).delete();
      }

    return(
        <Container>
            <h2>Items on your cart</h2>
            <h3>Currently you have {itemNum} course on your cart</h3>
            <Content>
                {cartItems &&
                    cartItems.map((movie, key) => (
                        <Wrap key={key}>
                        ...
                        <Link to={`/detail/` + movie.id}>
                            ...
                            <h3 onClick={() => onRemove(movie.id)}>Remove from cart</h3>
                            </div>
                        </Link>
                        </Wrap>
                ))}
